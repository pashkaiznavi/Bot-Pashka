                ...
                if messages["count"] >= 1:
                    ...
                    if "кубик" in body.lower():
                            ...
                            if user_cube == cube:
                                vk.method("messages.send", {"peer_id": id,
                                                            "message": str(user_name) + ", вы угадали! 😯 Выйгрыш " + str(
                                                                format(user_win,',')) + "$\n" +"💰Ваш баланс: " + str(format((int(result[0][3]) + user_win),',')), "random_id": random.randint(1, 2147483647)})
                                q = connection.cursor()
                                q.execute("SELECT * FROM BotUsers WHERE vk_id = %s" % (id))
                                result = q.fetchall()
                                q.execute(
                                    "UPDATE BotUsers SET Balance = '%s' WHERE vk_id = '%s'" % (int(result[0][3]) + user_win, id))
                                connection.commit()
                                connection.close()

                            else:
                                vk.method("messages.send", {"peer_id": id,
                                                            "message": user_name + ", вы проиграли!\n 🎲 Выпало число " + str(
                        ...
                        if len(result) == 0:
                            ...
                            q = connection.cursor()
                            q.execute("SELECT * FROM BotUsers WHERE vk_id = %s" % (id))
                            q.execute("UPDATE BotUsers SET nick = '%s' WHERE vk_id = '%s'" % (nick, id))
                            connection.commit()
                            connection.close()
                        ...
                        if len(result) == 0:
                            ...
                            if int(result[0][3]) >= int(sum):
                                ...
                                q.execute(
                                            "UPDATE BotUsers SET Balance = '%s' WHERE vk_id = '%s'" % (int(result[0][3]) - int(sum), id))
                                ...
                                q = connection.cursor()
                                q.execute("SELECT * FROM BotUsers WHERE vk_id = %s" % (chel))
                                result = q.fetchall()
                                
                                q = connection.cursor()
                                q.execute(
                                            "UPDATE BotUsers SET Balance = '%s' WHERE vk_id = '%s'" % (int(result[0][3]) + int(sum), chel))
                                connection.commit()
                                connection.close()
                                vk.method("messages.send",
                        ...
                        result = q.fetchall()
                        q.execute(
                                    "UPDATE BotUsers SET Balance = '%s' WHERE vk_id = '%s'" % (int(result[0][3]) + 100000, id))
                        connection.commit()
                        connection.close()
                        ...
                        if len(result) == 0:
                            ...
                            if int(result[0][3]) >= rate:
                                if kazino == 1:
                                    ...
                                    if coefficient == 1:
                                        ...
                                        connection = pypyodbc.connect('DRIVER={SQL Server};'
                                                                       'SERVER=.\SQLEXPRESS;'
                                                                          'DATABASE=Bot_PaSHKa;')
                                        q = connection.cursor()
                                        q.execute("UPDATE BotUsers SET Balance = '%s' WHERE vk_id = '%s'" % (int(money) +
                                                                                                                rate * 7, id))
                                        connection.commit()
                                        connection.close()
                                        vk.method("messages.send",
                                    ...
                                    vk.method("messages.send",
                                              {"peer_id": id, "message": "Вы проиграли " + str(format(rate,',')) + "😔\n" + "Ваш баланс: " + str(format((int(result[0][3]) - rate),',')) , "random_id": random.randint(1, 2147483647)})
                                    connection = pypyodbc.connect('DRIVER={SQL Server};'
                                        'SERVER=.\SQLEXPRESS;'
                                        'DATABASE=Bot_PaSHKa;')
                                    q = connection.cursor()
                                    q.execute("UPDATE BotUsers SET Balance = '%s' WHERE vk_id = '%s'" % (int(result[0][3]) - rate, id))
                                    connection.commit()
                                    connection.close()
                            else:
                                vk.method("messages.send",
                        ...
                        if len(result) == 0:
                            ...
                            q.execute("SELECT * FROM BotUsers WHERE nick = %s" % str(nick))
                            result = q.fetchall()
                            id1 = result[0][2]
                            if len(nick) != 0:
                                vk.method("messages.send", {"peer_id": id,
                        ...
                        try:
                            ...
                            if len(result) == 0:
                                ...
                                if realty == "дом":
                                    ...
                                    if int(money) >= 100000:
                                        if ownment != None:
                                            q.execute("UPDATE BotUsers SET Ownment = '%s' WHERE vk_id = '%s'" % (
                                            str(ownment) + "1,", id))
                                            q.execute(
                                                "UPDATE BotUsers SET Balance = '%s' WHERE vk_id = '%s'" % (int(money) - 100000,
                                                                                                              id))
                                            connection.commit()
                                            connection.close()
                                            vk.method("messages.send", {"peer_id": id, "message": "Вы купили дом&#127968;!\n" + "Ваш баланс: " + str(format((int(result[0][3])-100000),',')), "random_id": random.randint(1, 2147483647)})
                                        else:
                                            q.execute("UPDATE BotUsers SET Ownment = '%s' WHERE vk_id = '%s'" % ("1,", id))
                                            q.execute(
                                                "UPDATE BotUsers SET Balance = '%s' WHERE vk_id = '%s'" % (int(money) - 100000,
                                                                                                              id))
                                            connection.commit()
                                        ...
                                        vk.method("messages.send",
                                                  {"peer_id": id, "message": "Недостаточно денег для покупки!\n" + "Вам не хватает: " + str(format((100000 - int(result[0][3])),',')), "random_id": random.randint(1, 2147483647)})
                                elif realty == "машина":
                                    connection = pypyodbc.connect('DRIVER={SQL Server};'
                                    'SERVER=.\SQLEXPRESS;'
                                    'DATABASE=Bot_PaSHKa;')
                                    ...
                                    if int(money) >= 50000:
                                        if ownment != None:
                                            ...
                                            str(ownment) + "2,", id))
                                            q.execute(
                                                "UPDATE BotUsers SET Balance = '%s' WHERE vk_id = '%s'" % (int(money) - 50000,
                                                                                                              id))
                                            connection.commit()
                                            connection.close()
                                            vk.method("messages.send", {"peer_id": id, "message": "Вы купили машину!&#128664;\n" + "Ваш баланс: " + str(format((int(result[0][3])-50000),',')), "random_id": random.randint(1, 2147483647)})
                                        else:
                                            q.execute("UPDATE BotUsers SET Ownment = '%s' WHERE vk_id = '%s'" % ("2,", id))
                                            q.execute(
                                                "UPDATE BotUsers SET Balance = '%s' WHERE vk_id = '%s'" % (int(money) - 50000,
                                                                                                              id))
                                            connection.commit()
                                        ...
                                        vk.method("messages.send",
                                                  {"peer_id": id, "message": "Недостаточно денег для покупки!\n" + "Вам не хватает: " + str(format((50000 - int(result[0][3])),',')), "random_id": random.randint(1, 2147483647)})
                                elif realty == "дача":
                                    connection = pypyodbc.connect('DRIVER={SQL Server};'
                                        'SERVER=.\SQLEXPRESS;'
                                        'DATABASE=Bot_PaSHKa;')
                                    ...
                                    if int(money) >= 40000:
                                        if ownment != None:
                                            ...
                                            str(ownment) + "3,", id))
                                            q.execute(
                                                "UPDATE BotUsers SET Balance = '%s' WHERE vk_id = '%s'" % (int(money) - 40000,
                                                                                                              id))
                                            connection.commit()
                                            connection.close()
                                            vk.method("messages.send", {"peer_id": id, "message": "Вы купили дачу!&#127969;\n" + "Ваш баланс: " + str(format((int(result[0][3])-40000),',')), "random_id": random.randint(1, 2147483647)})
                                        else:
                                            q.execute("UPDATE BotUsers SET Ownment = '%s' WHERE vk_id = '%s'" % ("3,", id))
                                            q.execute(
                                                "UPDATE BotUsers SET Balance = '%s' WHERE vk_id = '%s'" % (int(money) - 40000,
                                                                                                              id))
                                            connection.commit()
                                        ...
                                        vk.method("messages.send",
                                                  {"peer_id": id, "message": "Недостаточно денег для покупки!\n" + "Вам не хватает: " + str(format((40000 - int(result[0][3])),',')), "random_id": random.randint(1, 2147483647)})
                        except:
